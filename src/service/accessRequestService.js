...
const exportAccessRequestConfig = async apiConfig => {
    ...
    if (grantRequestApprovalSchemes && grantRequestApprovalSchemes.includes("workgroup:")) {
        ...
        const endIndex =
            grantRequestApprovalSchemes.indexOf(",", startIndex) !== -1
                ? grantRequestApprovalSchemes.indexOf(",", startIndex)
                : grantRequestApprovalSchemes.length;
...
const updateAccessRequestConfig = async apiConfig => {
    ...
    const accessRequestConfigFilePaths = walk("./build/config/ACCESS_REQUEST_CONFIG");

    //Iterate each source and pass it to migrateSource
    for (const accessRequestConfigFilePath of accessRequestConfigFilePaths) {
        const accessRequestConfigSource = fs.readFileSync(accessRequestConfigFilePath);
...
export { exportAccessRequestConfig, updateAccessRequestConfig };
