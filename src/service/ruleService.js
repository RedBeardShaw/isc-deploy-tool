...
const migrateCloudRules = async apiConfig => {
    ...
    if (rulesToDeploy.length > 0) {
        ...
        if (
            ruleImportResponse.data &&
            ruleImportResponse.data.results &&
            ruleImportResponse.data.results.RULE &&
            ruleImportResponse.data.results.RULE.errors
        ) {
            if (ruleImportResponse.data.results.RULE.errors.length > 0) {
                winston.error(
                    clc.red(
                        `Error importing cloud rules via SP-Config:\n${JSON.stringify(ruleImportResponse.data.results.RULE.errors, null, 4)}`
                    )
                );
                process.exit(1);
            }
...
const migrateConnectorRule = async (apiConfig, connectorRuleJson) => {
    ...
    if (!currentTargetConnectorRule) {
        ...
        winston.info(
            `Updating existing connector rule: ${currentTargetConnectorRule.name} (${currentTargetConnectorRule.id})`
        );
...
export { exportConnectorRules, exportCloudRules, getAllRules, migrateCloudRules, migrateConnectorRules };
