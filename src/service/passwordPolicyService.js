...
const migratePasswordPolicy = async (apiConfig, passwordPolicyJson) => {
    ...
    if (!currentTargetPasswordPolicy) {
        ...
        winston.info(
            `Updating existing password policy: ${currentTargetPasswordPolicy.name} (${currentTargetPasswordPolicy.id})`
        );
...
export { exportPasswordPolicies, getAllPasswordPolicies, migratePasswordPolicies };
