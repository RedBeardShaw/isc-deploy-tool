...
const updateBranding = async (apiConfig, targetEnvName) => {
    ...
    for (const brandingConfigFilePath of brandingConfigFilePaths) {
        ...
        if (fs.existsSync(logoFileName)) {
            ...
            winston.info(
                clc.yellow(
                    `PNG image does not exist in ./assets directory for environment [${targetEnvName}]. Branding image will not be uploaded. Full file name should be ./assets/${targetEnvName}.png`
                )
            );
        }
            ...
            if (!currentTargetBrandingConfig) {
                ...
                try {
                    const createBrandingConfigResponse = await brandingApi.createBrandingItemV1({
                        ...
                        actionButtonColor:
                            localBrandingConfig.actionButtonColor != null
                                ? localBrandingConfig.actionButtonColor
                                : undefined,
                        activeLinkColor:
                            localBrandingConfig.activeLinkColor != null
                                ? localBrandingConfig.activeLinkColor
                                : undefined,
                        emailFromAddress:
                            localBrandingConfig.emailFromAddress != null
                                ? localBrandingConfig.emailFromAddress
                                : undefined,
                        fileStandard: logoFile != null ? logoFile : undefined,
                        loginInformationalMessage:
                            localBrandingConfig.loginInformationalMessage != null
                                ? localBrandingConfig.loginInformationalMessage
                                : undefined,
                        navigationColor:
                            localBrandingConfig.navigationColor != null
                                ? localBrandingConfig.navigationColor
                                : undefined,
                    });
...
export { exportBranding, updateBranding };
