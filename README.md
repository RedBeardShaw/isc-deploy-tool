...
config
 ...
 ┣ CLOUD_RULE
 ┃ ┣ Epic SER Multivalued Update.json
 ┃ ┗ Epic SER Multivalued Update.source.bsh
 ┣ CONNECTOR_RULE
 ┃ ┣ AfterModifyRule.json
 ┃ ┗ AfterModifyRule.src
 ┣ FORM_DEFINITION
 ┃ ┗ PrivilegedAccountDescriptionTransform.json
 ┣ GOVERNANCE_GROUP
 ┃ ┗ Corp Governance Group.json
 ┣ IDENTITY_OBJECT_CONFIG
 ...
 ┃ ┃ ┗ HR.json
 ┣ NOTIFICATION_TEMPLATE
 ┃ ┗ Access Profile Owner Approval Notification.json
 ┣ ORG_CONFIG
 ┃ ┣ LOCKOUT_ORG_CONFIG.json
 ┃ ┣ NETWORK_ORG_CONFIG.json
 ┃ ┣ PASSWORD_ORG_CONFIG.json
 ┃ ┣ PUBLIC_IDENTITIES_ORG_CONFIG.json
 ┃ ┣ SERVICE_PROVIDER_ORG_CONFIG.json
 ┃ ┗ SESSION_ORG_CONFIG.json
 ┣ PASSWORD_INSTRUCTION
 ┃ ┗ reset-password-enter-password.json
 ┣ PASSWORD_POLICY
 ┃ ┗ Default.json
 ┣ ROLE
 ┃ ┗ Accounts Payable Analyst.json
 ┣ SOURCE
 ┃ ┣ Active Directory
 ┃ ┃ ┣ AGGREGATION_SCHEDULE
 ┃ ┃ ┃ ┣ ACCOUNT_AGGREGATION.json
 ┃ ┃ ┃ ┗ GROUP_AGGREGATION.json
...
Access Profiles and Roles are last in the deployment sequence because the deployment of an access profile or role with an associated entitlement will be depedent on the entitlement existing in the tenant. For the scenario where a source is being created alongside access profiles or roles that contain entitlements for that source, the deployment will FAIL since the source will not have any entitlements. For this case, run the deployment once to create the source, then execute an entitlement aggregation on the source, and run the deployment again to create the access profiles or roles.
